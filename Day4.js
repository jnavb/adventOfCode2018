...
const test = [
	{moment: "040500", id: "2633"},
	{moment: "040502", id: "2459"},
	{moment: "040526"},
	{moment: "040533"},
	{moment: "040540"},
	{moment: "040541"},
	{moment: "040600", id: "3121"},
	{moment: "040618"},
	{moment: "040640"},
	{moment: "040700", id: "2897"},
	{moment: "040704"},
	{moment: "040706"},
	{moment: "040727"},
	{moment: "040758"}
...
const beginShift = (str) => !!str.match('shift');
const getMonth = (str) => str.substring(6, 8);
const getDay = (str) => str.substring(9, 11);
const getMinute = (str) => (str.substring(12, 14) === '00') ? str.substring(15, 17) : '00';
const getId = (str) => {
	const id = str.match(/#[0-9]*/);
	return (id) ? id[0].substring(1) : null;
}
const getMinuteFromMoment = (str) => str.substring(4);

const orderTimestamps = (input) => {
	const timestamps = input.map(timestamp => formatTimestamp(timestamp));
	return timestamps.sort((a, b) => {
		const great = a.moment;
		const less = b.moment;
		return ((great < less) ? -1 : ((great > less) ? 1 : 0));
	});
}
...
const formatTimestamp = (timestamp) => {
	...
	const id = getId(timestamp);
	const res = {
		moment: month + day + minute
	}
	if (id) res.id = id;
...
const formatInShifts = (timestamps) => {
	...
	for (const [index, value] of timestamps.entries()) {
		if (index === 0) {
			startsShift = getMinuteFromMoment(value.moment);
			guardId = value.id;
			continue;
		}
		if (index === timestamps.length) {
			endShift = finishSleep = getMinuteFromMoment(value.moment);
			guards = recordGuardTime(guards, { id: guardId, start: parseInt(startsShift), end: parseInt(endShift) });
			guards = recordSleepTime(guards, { id: guardId, start: parseInt(startsSleep), end: parseInt(finishSleep) });
		}

		if (value.id) {
			endShift = getMinuteFromMoment(value.moment);
			guards = recordGuardTime(guards, { id: guardId, start: parseInt(startsShift), end: parseInt(endShift) });
			...
			startsSleep = getMinuteFromMoment(value.moment);
			isAwake = false;
		} else {
			finishSleep = getMinuteFromMoment(value.moment);
			guards = recordSleepTime(guards, { id: guardId, start: parseInt(startsSleep), end: parseInt(finishSleep) });
		}
	}
	return guards;
...
const recordGuardTime = (guards, { id, start, end }) => {
	// PERFECT
	const guardsArr = guards;
	const time = (start < end) ? end - start : 60 - start + end;
	if (!guardsArr[id]) {
		guardsArr[id] = {};
	}
	guardsArr[id].totaltime = guardsArr[id].totaltime + time || time;
	return guardsArr;
}

const recordSleepTime = (guards, { id, start, end }) => {
	// PERFECT
	const guardsArr = guards;
	const time = (start < end) ? end - start : 60 - start + end;
	if (!guardsArr[id]) {
	...
	guardsArr[id].sleepTime = guardsArr[id].sleepTime + time || time;
	
	if (!guardsArr[id].sleepSchedule) {
		guardsArr[id].sleepSchedule = new Array(60).fill(0);
	}
	if (start < end) {
		
		guardsArr[id].sleepSchedule = guardsArr[id].sleepSchedule.map((minute, index) => {
			return (start <= index && index < end) ? ++minute : minute;
		});
	} else {
		guardsArr[id].sleepSchedule = guardsArr[id].sleepSchedule.map((minute, index) => {
			return ((start <= index && index <= 59) || index < end) ? ++minute : minute;
		});
	}
	return guardsArr;
}
